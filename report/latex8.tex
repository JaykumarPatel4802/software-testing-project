...
The results of our experiment can be seen in Figure~\ref{fig:main}. Figure~\ref{fig:sub1} shows the accuracy and consistency of the code generated for each Leetcode question for ChatGPT4o. This model was able to provide code that passed all of the given test cases for each of the 3 trials that were performed for each problem, regardless of the difficulty level or problem categorization.

Figure~\ref{fig:sub2} shows the results for Gemini Flash 1.5. The results for this model were comparable to the results from ChatGPT4o, with only 3 incorrect solutions out of the 32 problems that were being tested. An interesting thing to note for this model is that the responses were consistent, even in its wrong answers. For the problems Gemini was unable to provide a correct solution for, it produced an incorrect response for all 3 trials, with each response generating the same error in the IDE. 
...
It was able to generate correct responses for most of medium and easy level questions, but the hard-type questions received more incorrect responses. This is probably due to the fact that  hard questions are more nuanced and have more edge cases to be aware of that the model was not able to capture in its code. In terms of problem category,
Arrays/Hashing and Two-pointer questions were answered more accuracy, and this is likely because these questions typically more widespread and easier to answer than DP/Graph problems. 
...
Figure \ref{fig:example} shows the results for problem categorization for each of the 3 models. Overall, all 3 models were able to categorize most, if not all, of the 32 questions that were tested. The model with the lowest accuracy was LLama 3, with an accuracy of 30/32. 
...
Mutation testing was performed on all 3 models in order to test their generalizability or adaptability, in the case that the exact versions of the LeetCode problems were present in the training dataset and already seen by the models during training. As seen in Figure~\ref{fig:mutation}, Gemini produced the same results as the original data, whereas ChatGPT and LLama saw a decrease in accuracy, with a far a larger decrease for LLama. This could be attributed to ChatGPT4o and Gemini being more finetuned or optimized for programming tasks compared to Llama, and are better at adapting to variations in problem definitions due to their larger model size and refined architecture. 


\subsection{Analysis of Results}
For our experiment, we saw that models with more parameters (ChatGPT4o and Gemini) performed better overall than models with less parameters (Llama 7 8B). Within the scope of our project, we were able to conclude that they may be a correlation between model size and the ability to generate accurate and consistent code for DSA problems. However, one thing to consider is that Gemini Flash 1.5 is a more lightweight model that was trained through model distillation from larger models (with sizes comparable to ChatGPT4o). Gemini Flash 1.5 therefore has far fewer parameters than ChatGPT4o, but has relatively comparable results. Therefore, though model size may impact performance, there is not a linear correlation between number of parameters and accuracy. Other factors may also play a role in model performance, such as model architecture, training time/methodologies, training data, context window, etc. 

Additionally, we noticed that ChatGPT4o and Gemini Flash 1.5 occasionally cited websites such as GitHub or LeetCode itself in its responses, which is an indication that these models utilize Retreival Augmented Generation (RAG). More advanced models are known to combine traditional querying/document retrieval with generative AI, which may also be a reason for their improved performance.
